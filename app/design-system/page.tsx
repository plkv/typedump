...
const ctrlBase: React.CSSProperties = {
  fontFamily: '"Strichpunkt Sans", sans-serif', cursor: 'pointer', transition: 'all 0.15s',
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {controls.map((c, i) => {
        ...
        if (c.type === 'text') return (
          <div key={i}>
            {lbl}
            <input value={c.value} onChange={e => c.onChange(e.target.value)} style={{ width: '100%', padding: '6px 10px', borderRadius: 8, border: '1px solid var(--gray-brd-prim)', background: 'var(--gray-fill-prim)', color: 'var(--gray-cont-prim)', fontSize: 12, fontFamily: '"Strichpunkt Sans", sans-serif', outline: 'none', boxSizing: 'border-box' }} />
          </div>
        )
        if (c.type === 'toggle') return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--gray-cont-prim)', fontFamily: '"Strichpunkt Sans", sans-serif' }}>{c.label}</span>
          </div>
        )
        return null
      })}
  ...
  return (
    <div style={{ borderRadius: 'var(--radius-card)', border: '1px solid var(--gray-brd-prim)', overflow: 'hidden', marginBottom: 20 }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--gray-brd-prim)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--gray-cont-prim)', fontFamily: '"Strichpunkt Sans", sans-serif' }}>{name}</span>
        <Mono>{cls}</Mono>
      </div>
    ...
    <Playground name="Card" cls=".v2-card" code={code}
      preview={
        <div className="v2-card" style={{ padding: Number(padding), width: 180, border: '1px solid var(--gray-brd-prim)' }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--gray-cont-prim)', fontFamily: '"Strichpunkt Sans", sans-serif', marginBottom: 4 }}>Card title</div>
          <div style={{ fontSize: 12, color: 'var(--gray-cont-tert)', fontFamily: '"Strichpunkt Sans", sans-serif' }}>Some content inside the card.</div>
        </div>
      }
      controls={
        <ControlPanel controls={[
    ...
    <Playground name="Dropdown" cls=".v2-dropdown" code={code}
      preview={
        <div className={style} style={{ display: 'flex', alignItems: 'center' }}>
          <select style={{ padding: '10px 36px 10px 12px', border: 'none', background: 'transparent', fontSize: 14, fontWeight: 500, fontFamily: '"Strichpunkt Sans", sans-serif', color: 'var(--gray-cont-prim)', cursor: 'pointer' }}>
    ...
    <div style={{ backgroundColor: 'var(--gray-surface-prim)', minHeight: '100vh' }}>
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '48px 24px 120px' }}>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 600, marginTop: 16, color: 'var(--gray-cont-prim)', fontFamily: '"Strichpunkt Sans", sans-serif' }}>
            Design System
          </h1>
          <p style={{ fontSize: 14, color: 'var(--gray-cont-sec)', marginTop: 8, fontFamily: '"Strichpunkt Sans", sans-serif' }}>
        ...
        <Section title="Colors — gray tokens">
          <p style={{ fontSize: 12, color: 'var(--gray-cont-tert)', marginBottom: 16, fontFamily: '"Strichpunkt Sans", sans-serif' }}>
        ...
        <Section title="Typography">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            {[
              { name: 'Strichpunkt Sans', family: '"Strichpunkt Sans"', note: '--font-inter · main UI font', weights: [400, 500, 600, 700, 900] },
          ...
          <div style={{ marginTop: 24, borderRadius: 12, border: '1px solid var(--gray-brd-prim)', overflow: 'hidden' }}>
            ...
            {[
              { cls: '.text-sidebar-title', sample: 'Sidebar title', spec: 'Strichpunkt Sans 14px/14px 500, ss03+cv06+cv11' },
              { cls: '.text-font-name',     sample: 'Font name',     spec: 'Strichpunkt Sans 14px/20px 500, ss03+cv06+cv11' },
              { cls: '.text-menu',          sample: 'Menu item',     spec: 'Strichpunkt Sans 14px/20px 500 (same as .text-font-name)' },
              { cls: '.text-author',        sample: 'by Some Author', spec: 'Strichpunkt Sans 14px/20px 500 · color: --gray-cont-tert' },
              ...
              <div key={cls} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 16px', borderBottom: i < arr.length - 1 ? '1px solid var(--gray-brd-prim)' : 'none' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--gray-cont-tert)', fontFamily: '"Strichpunkt Sans", sans-serif', textAlign: 'right' }}>{spec}</div>
              </div>
            ))}
          </div>
        </Section>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={token} style={{ borderRadius: 12, border: '1px solid var(--gray-brd-prim)', padding: '16px 20px', display: 'flex', gap: 16, alignItems: 'center' }}>
                ...
                <div>
                  <Mono>{token}</Mono>
                  <div style={{ fontSize: 12, color: 'var(--gray-cont-prim)', fontFamily: '"Strichpunkt Sans", sans-serif', marginTop: 4 }}>{value}</div>
                  <div style={{ fontSize: 11, color: 'var(--gray-cont-tert)', fontFamily: '"Strichpunkt Sans", sans-serif', marginTop: 2 }}>{use}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <Section title="Spacing">
          <p style={{ fontSize: 12, color: 'var(--gray-cont-sec)', marginBottom: 16, fontFamily: '"Strichpunkt Sans", sans-serif' }}>
